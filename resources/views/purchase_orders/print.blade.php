...
<head>
    ...
    <title>Print Purchase Order {{ $order->code }}</title>

    <style>
        /**
         * Purchase Order PRINT VIEW (A4) – Professional layout
         * - Standalone HTML (no ERP sidebar/header)
         * - A4 preview on screen, clean output on print
         * - Fixed table widths + safe wrapping for long text
        ...
        * { box-sizing: border-box; }
        ...
        /* Simple toolbar (hidden on print) */
        .toolbar {
            position: sticky;
            top: 0;
            z-index: 10;
        ...
        /* Safer wrapping for long strings (even without spaces) */
        .wrap {
            white-space: normal;
        ...
        /* Prevent row splitting (best effort) */
        ...
        @media print {
            ...
            /* IMPORTANT: rely on @page margins; don't double-pad */
            .page-inner {
                padding: 0;
            }
        }
...
@php
    // Company / Vendor blocks (safe fallbacks if modules differ)
    ...
    // Logo
    $logoPublicPath = public_path('images/ems-logo.png');
    $logoUrl = (is_string($logoPublicPath) && file_exists($logoPublicPath))
        ? asset('images/ems-logo.png')
        : null;
    ...
    // Show a single GST column to keep within A4 width
...
<div class="toolbar">
    <div>
        <div class="title">Purchase Order {{ $order->code }} — Print Preview</div>
        <div class="subtitle">Use the Print button for clean A4 output.</div>
    </div>
    <div class="actions">
        <a class="btn" href="{{ route('purchase-orders.show', $order) }}">Back to PO</a>
                        ...
                        <tr>
                            @if($logoUrl)
                                <td style="width: 60px; padding-right: 8px;">
                                    <img src="{{ $logoUrl }}" alt="{{ $companyName }}" style="height: 42px; width: auto;">
                                </td>
                            @endif
        ...
        <table class="items-table {{ $itemsFontClass }} w-100" style="margin-top: 4px;">
            ...
            @foreach($items as $index => $item)
                @php
                    ...
                    if (filled($item->brand ?? null)) { $specParts[] = 'Brand: ' . $item->brand; }
                    if (filled($item->grade ?? null)) { $specParts[] = 'Grade: ' . $item->grade; }
                    if ((float) ($item->length_mm ?? 0) > 0) { $specParts[] = 'L: ' . rtrim(rtrim(number_format((float) $item->length_mm, 2), '0'), '.') . 'mm'; }
                    if ((float) ($item->width_mm ?? 0) > 0) { $specParts[] = 'W: ' . rtrim(rtrim(number_format((float) $item->width_mm, 2), '0'), '.') . 'mm'; }
                    if ((float) ($item->thickness_mm ?? 0) > 0) { $specParts[] = 'T: ' . rtrim(rtrim(number_format((float) $item->thickness_mm, 2), '0'), '.') . 'mm'; }
                    if ((float) ($item->weight_per_meter_kg ?? 0) > 0) { $specParts[] = 'Wt/m: ' . rtrim(rtrim(number_format((float) $item->weight_per_meter_kg, 3), '0'), '.') . 'kg'; }
                    if ((float) ($item->qty_pcs ?? 0) > 0) { $specParts[] = 'Qty(pcs): ' . rtrim(rtrim(number_format((float) $item->qty_pcs, 3), '0'), '.'); }
                    if (filled($item->section_profile ?? null)) { $specParts[] = 'Section: ' . $item->section_profile; }
                    if ($showDisc && (float) ($item->discount_percent ?? 0) > 0.0001) { $specParts[] = 'Disc: ' . number_format((float) $item->discount_percent, 2) . '%'; }
