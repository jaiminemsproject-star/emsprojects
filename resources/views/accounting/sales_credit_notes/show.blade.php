...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-0">Sales Credit Note: {{ $note->note_number }}</h4>
            <div class="text-muted small">Client: {{ $note->client?->name }} | Date: {{ optional($note->note_date)->format('Y-m-d') }}</div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('accounting.sales-credit-notes.index') }}" class="btn btn-light btn-sm">Back</a>
            @if($note->status==='draft')
                <a href="{{ route('accounting.sales-credit-notes.edit', $note) }}" class="btn btn-outline-primary btn-sm">Edit</a>
                <form method="POST" action="{{ route('accounting.sales-credit-notes.post', $note) }}" class="d-inline">
                    @csrf
                    <button class="btn btn-success btn-sm" onclick="return confirm('Post this Credit Note to accounts?')">Post</button>
                </form>
            @endif
            @if($note->status==='posted')
                <button class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#cancelCNModal">Cancel</button>
            @endif
        </div>
    </div>
    ...
    @if($note->status==='posted')
    <div class="modal fade" id="cancelCNModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Cancel Credit Note</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form method="POST" action="{{ route('accounting.sales-credit-notes.cancel', $note) }}">
                    @csrf
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-danger btn-sm" onclick="return confirm('Cancel this credit note?')">Confirm Cancel</button>
                    </div>
                </form>
            </div>
        </div>
