...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">{{ ($note->exists ? 'Edit' : 'Create') }} Purchase Debit Note</h4>
    ...
    <form method="POST" action="{{ $note->exists ? route('accounting.purchase-debit-notes.update', $note) : route('accounting.purchase-debit-notes.store') }}">
        @csrf
        @if($note->exists)
            @method('PUT')
        @endif
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="supplier_id" class="form-select form-select-sm" required>
                            ...
                            @foreach($suppliers as $s)
                                <option value="{{ $s->id }}" @selected((int)old('supplier_id', $note->supplier_id) === (int)$s->id)>
                                    {{ $s->name }} ({{ $s->code }})
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                <strong>Lines (Accounts to CREDIT)</strong>
            ...
            <div class="card-body table-responsive">
                <table class="table table-sm align-middle" id="linesTable">
                    <thead>
                    <tr>
                        <th style="width: 30%">Credit Account</th>
                        <th>Description</th>
                ...
                <div class="text-muted small">
                    Debit Note posts: <strong>Dr Supplier</strong>, <strong>Cr selected accounts</strong>, and <strong>Cr Input GST</strong> (reversal).
                </div>
            </div>
        </div>
        ...
        <div class="d-flex gap-2">
            ...
            @if($note->exists)
                <a href="{{ route('accounting.purchase-debit-notes.show', $note) }}" class="btn btn-light">Cancel</a>
            @endif
        </div>
    </form>
</div>
