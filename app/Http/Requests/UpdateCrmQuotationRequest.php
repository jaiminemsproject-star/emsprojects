...
class UpdateCrmQuotationRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can('crm.quotation.update') ?? false;
    }
    ...
    public function rules(): array
    {
        ...
        $revisionNo  = $quotation?->revision_no ?? 0;

        $quoteMode  = $this->input('quote_mode', $quotation?->quote_mode ?? 'item');
        ...
        return [
            'code' => [
                ...
                Rule::unique('crm_quotations', 'code')
                    ->where(fn ($q) => $q->where('revision_no', $revisionNo))
                    ->ignore($quotationId),
            ],
